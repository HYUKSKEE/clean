  ...
  return (
    ...
    >
      <div className="container-page">
        <SectionHeading
          eyebrow="Contact"
          title={<span id="contact-title">무료 견적 문의</span>}
          description="상담과 출장 견적은 모두 무료입니다. 전화가 편하시면 바로 연락 주세요."
        />
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_1.15fr] lg:gap-8">
          <div className="flex flex-col gap-6">
            <div className="rounded-2xl border border-line bg-white p-6 lg:p-8">
              ...
              <a
                href={telHref}
                className="mt-4 inline-flex min-h-13 items-center gap-2.5 rounded-full bg-brand px-6 text-xl font-extrabold text-ink transition-colors hover:bg-brand-strong"
              >
              ...
              <ol className="mt-5 flex flex-col gap-5">
                {steps.map((step, index) => (
                  <li key={step.title} className="flex gap-3.5">
                    <span className="grid size-8 shrink-0 place-items-center rounded-full bg-brand-soft text-sm font-extrabold text-brand-deep">
                      {index + 1}
                    </span>
                    <div>
